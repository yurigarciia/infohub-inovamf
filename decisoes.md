...
**E-mail (revisão da Q7):** em vez do Resend, usamos o **mail-service** próprio da dupla (`POST /emails` com `x-api-key`, corpo em HTML, resposta `202 queued` — fire-and-forget, ele mesmo refaz até 3x). Fica atrás da interface `EmailSender` (`server/src/modules/notifications/email-senders.ts`); sem `MAIL_API_KEY` cai no `ConsoleEmailSender`. Decisões: (1) o envio roda dentro da request do usuário, então há timeout de 5 s e um "circuito" (30 s) que faz as chamadas seguintes falharem na hora se o serviço cair — a falha vira `email_notifications.status = FAILED` sem derrubar o fluxo; (2) os e-mails do seed são fictícios em domínios que podem existir, então `MAIL_SKIP_DOMAINS` impede o envio real para eles (só log, `provider_message_id = demo-skip-…`); (3) não há endpoint para consultar entrega, então `SENT` significa "aceito e enfileirado", não "entregue"; (4) a chave é segredo de deploy (env do Coolify), nunca versionada.
