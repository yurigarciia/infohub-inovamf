...
### Ticket: T-FE-34 Mesmo dialog de detalhes na área do aluno
- **Priority:** P2
- **Status:** Done
- **Scope:** Estender o padrão do T-FE-33 (linha resumo clicável → dialog com detalhes) pra área do aluno — o pedido original foi genérico ("clicar pra ver detalhes"), só implementado antes na tela de equipe do admin/mentor.
- **Acceptance Criteria:** `TaskCard` (área do aluno) vira linha resumo clicável, com dialog mostrando descrição, histórico de entregas com link e comentário do mentor; envio de entrega continua funcionando (agora dentro do dialog).
- **Validation Steps:** Abrir `/aluno`, clicar numa tarefa concluída (conferir descrição + link da entrega) e numa pendente (conferir formulário de envio dentro do dialog); checar 375px sem overflow.
- **Notes:** `task-card.tsx` reescrito no mesmo padrão de `task-review-item.tsx` (T-FE-33) — `DialogTrigger` como linha resumo, `DialogContent` com descrição, `task.submissions` completo (não só a atual, já que `TaskWithTeam` estende `TaskWithDetails`) com link e status de revisão por versão, comentário do mentor, e `TaskSubmissionForm` inline quando a tarefa ainda está pendente (`canSubmit`). Ao enviar a entrega, o dialog fecha (`setIsOpen(false)`) além de recarregar a lista, evitando ficar com o dialog aberto mostrando dados desatualizados. Validado com Playwright: tarefa aprovada mostra descrição + link da entrega (`v1 (link)` — Pitch Vídeo); tarefa pendente abre o formulário de envio de arquivo dentro do dialog; zero overflow em 375px. Sem erros de console.
